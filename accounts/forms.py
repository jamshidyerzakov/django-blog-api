...
class ReaderAdminCreationForm(forms.ModelForm):
    """
    ...
    def save(self, commit=True):
        # Save the provided password in hashed format
        reader = super(ReaderAdminCreationForm, self).save(commit=False)
...
class ReaderAdminChangeForm(forms.ModelForm):
    """A form for updating readers. Includes all the fields on
    the user, but replaces the password field with admin's
    password hash display field.
    """
    password = ReadOnlyPasswordHashField()
...
class AuthorAdminChangeForm(forms.ModelForm):
    """A form for updating authors. Includes all the fields on
    the user, but replaces the password field with admin's
    password hash display field.
    """
    password = ReadOnlyPasswordHashField()
